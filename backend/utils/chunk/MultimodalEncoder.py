...
class MultimodalEncoder:
    ...
    def save_to_chroma(self, 
        ...
        for i, item in enumerate(encoded_chunks):
            ...
            if embedding is None:
                # 即使向量化失败（如网络原因），也使用全零虚拟向量存储到 ChromaDB
                # 这样可以确保管理员在后台能预览到切分好的 Chunks
                # 根据模型动态选择虚拟向量维度
                dim = 1152 if self.model == "tongyi-embedding-vision-plus" else 1024
                embedding = [0.0] * dim
            ...
            # 兼容旧版本：确保拥有必要的检索元数据字段
            meta['chunk_id'] = chunk_id
            meta['modality_type'] = chunk.get('modality_type', 'text')
            meta['is_active'] = True  # Phase 1: 文件生命周期管理，默认启用
    ...
    def query(self, 
        ...
        if modality_filter:
            conditions.append({"modality_type": modality_filter})
